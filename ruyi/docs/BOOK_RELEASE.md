...
- **书稿不随仓库开源（2026-07-04，Afra 拍板，推翻 2026-07-03 的 A 方案）**：书稿唯一存放地为 `D:\notes\RuyiBookCourse\图书\AI 开发\大鹏 RAG 实战：如意知识库工厂\`，不进本仓库。
- `ruyi/eval/corpus/` 是书稿的**本地同步副本**，已加入 `.gitignore` 不再入库；`ruyi-sync-corpus.ps1` 仍照常在有书稿的机器上跑，只影响本地评测语料。没有书稿的机器可自备任意 md 语料 + 对应 testset 跑评测（见 `ruyi/eval/README.md`）。
- 保留的限制不变：**现役钓鱼题原文不写进书稿**（评测自指污染教训，见书 ch11）。
- 历史重写已执行（2026-07-04，Afra 拍板）：用范围限定的 `git filter-branch` 从 2026-07-04 之前的 12 个提交中彻底移除 13 章书稿文件，`main`/`dev` 与 4 个 bk 标签全部重写强推。**这是「tag 永不移动」规则的一次性例外**，新旧 tag 对照：bk-ch01 `1dc26e72`→`0e3b3f4d`、bk-ch02 `2f681e6c`→`50fd13d6`、bk-ch03 `890d135c`→`ed9425bc`、bk-ch11 `7ab3d120`→`fa57465e`。重写前全量备份存 `D:\ai\RuyiDocsGPT-pre-rewrite-20260704.bundle`（含书稿，勿公开）。
- 已知残留：GitHub 服务端可能在其垃圾回收前仍可按旧提交哈希直接访问旧对象（可提 GitHub Support 工单请求彻底清除）；2026-07-04 前克隆过仓库的人本地仍持有旧历史，无法追回。
