...
```text
eval/
├─ testset.json    31 题(25 语料内 + 6 语料外钓鱼题) + 关键词判分 + 拒答措辞
...
一周八场"措辞漏词"冤案换来的分层设计，从便宜到贵：

1. **措辞库**（45 条固定短语）：零成本，秒判，但对非确定性输出是打地鼠；
2. **正则模式**（否定词+覆盖动词组合，见 `run_eval.py` 的 `REFUSAL_PATTERNS`）：一条顶几十条措辞，历史全部冤案原话都能兜住；
3. **LLM judge**（`--judge` 开启）：语料外题让大模型终审"诚实拒答还是编造事实"，语义级判定，连"恕难奉告"这种生僻说法都能判对。judge 读根目录 `.env` 的模型配置，失败自动回落启发式；报告会记录 judge 与启发式的分歧次数。

日常回归用默认启发式（快、免 API 费）；出红灯拿不准、或交付验收时，加 `--judge` 终审：

```powershell
.\.venv\Scripts\python.exe -X utf8 .\ruyi\eval\run_eval.py --active-docs <source_id> --judge
```
